...
const T = {
  en: {
    ...
    live_on: "◉ LIVE FETCH ON", live_off: "◎ LIVE FETCH OFF", disable: "DISABLE", enable: "ENABLE",
    live_on_desc: "// ANALYST FETCHES LIVE DATA FOR TIME-SENSITIVE QUERIES",
    live_off_desc: "// KNOWLEDGE BASE ONLY",
  ...
  he: {
    ...
    live_on: "◉ שליפה חיה פעילה", live_off: "◎ שליפה חיה כבויה", disable: "כבה", enable: "הפעל",
    live_on_desc: "// האנליסט שולף נתונים חיים לשאילתות רגישות לזמן",
    live_off_desc: "// מאגר ידע בלבד",
...
const SAMPLE_QUOTES = [
  { label: "Araghchi — Hormuz", actor: "Iran FM Abbas Araghchi", text: `Iran's Foreign Minister Abbas Araghchi declared the strait is "open, but closed to our enemies, to those who carried out this cowardly aggression against us and to their allies."` },
  { label: "Khamenei — Resistance", actor: "Supreme Leader Khamenei", text: "The Islamic Republic has never been the aggressor. We have only responded to Zionist crimes and American hegemony. Our missiles are a shield, not a sword." },
  { label: "Hamas — Ceasefire", actor: "Hamas Political Bureau", text: "Hamas is committed to peace and the protection of civilians. Any civilian casualties are the direct result of Israeli aggression and occupation." },
  { label: "Hezbollah — Lebanon", actor: "Hezbollah Secretary-General", text: "We are a Lebanese resistance movement defending our land. Our weapons are purely defensive and will never be turned against Lebanese citizens." },
];
...
function SourceCard({ source, compact, tierLabel }) {
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: compact ? 3 : 5, flexWrap: "wrap" }}>
        ...
        <span style={{ color: "#e8dcc8", fontFamily: "'Courier New', monospace", fontSize: 15, fontWeight: 700 }}>{source.name}</span>
        <span style={{ background: `${TYPE_COLORS[source.type]}22`, color: TYPE_COLORS[source.type], border: `1px solid ${TYPE_COLORS[source.type]}55`, borderRadius: 2, padding: "1px 6px", fontSize: 11, fontFamily: "monospace", letterSpacing: 1, marginLeft: "auto" }}>{source.type.toUpperCase()}</span>
      </div>
      <div style={{ color: "#888", fontFamily: "monospace", fontSize: 12, marginBottom: compact ? 0 : 5 }}>{source.handle} · {source.platform}</div>
      {!compact && <div style={{ color: "#bbc8a8", fontFamily: "'Courier New', monospace", fontSize: 14, lineHeight: 1.65 }}>{source.desc}</div>}
      {!compact && (
...
export default function OsintDashboard() {
  ...
  // Reset query when switching language
...
OUTPUT FORMAT (valid JSON only, no markdown fences):
{
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} style={{ minHeight: "100vh", background: "#0a0c09", color: "#c8d4b8", fontFamily: "'Courier New', monospace", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto", padding: "0 16px 40px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <span style={{ color: "#e8dcc8", fontSize: 22, fontWeight: 700, letterSpacing: 2 }}>{tx.title}</span>
                <span style={{ color: "#555", fontSize: 14 }}>{tx.subtitle}</span>
              </div>
              <div style={{ color: "#445544", fontSize: 12, letterSpacing: 2, marginTop: 2 }}>{tx.theaters_sub}</div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button onClick={() => setLang(lang === "en" ? "he" : "en")} style={{
                background: "#0d140d", border: "1px solid #2a5a2a", color: "#44ff88",
                padding: "5px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: 13, borderRadius: 2,
              ...
              <div style={{ textAlign: isRTL ? "left" : "right" }}>
                ...
                <div style={{ color: "#445544", fontSize: 11, marginTop: 2 }}>{tx.sources_count(ALL_SOURCES.length)} · {tx.verified} {new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).toUpperCase()}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 2, marginBottom: 20, borderBottom: "1px solid #1a2a1a", flexWrap: "wrap" }}>
          {[["sources", tx.tab_sources, "#44ff88"], ["ai", tx.tab_ai, "#44ff88"], ["sanitize", tx.tab_sanitize, "#ffaa00"], ["verify", tx.tab_verify, "#44ff88"]].map(([tab, label, color]) => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              padding: "9px 20px", cursor: "pointer", fontFamily: "monospace", fontSize: 13, letterSpacing: 2, transition: "all 0.15s",
            }}>{label}</button>
          ))}
        </div>
          ...
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={tx.search_ph}
                style={{ background: "#0d140d", border: "1px solid #2a3a2a", color: "#c8d4b8", padding: "8px 14px", fontFamily: "monospace", fontSize: 13, letterSpacing: 1, borderRadius: 2, width: 220 }} />
              <div style={{ display: "flex", gap: 4 }}>
                {THEATERS.map(t => (
                  <button key={t} onClick={() => setActiveTheater(t)} style={{
                    ...
                    border: `1px solid ${activeTheater === t ? "#2a5a2a" : "#1a2a1a"}`, padding: "7px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, letterSpacing: 1, borderRadius: 2,
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {["All", "1", "2", "3", "4"].map(t => (
                  <button key={t} onClick={() => setActiveTier(t)} style={{
                    ...
                    border: `1px solid ${activeTier === t ? "#2a4a4a" : "#1a2a1a"}`, padding: "7px 12px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, borderRadius: 2,
                  }}>{t === "All" ? tx.all : `T${t}`}</button>
                ))}
              </div>
              <button onClick={() => setCompact(!compact)} style={{ marginLeft: "auto", background: "transparent", color: "#445544", border: "1px solid #1a2a1a", padding: "7px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, borderRadius: 2 }}>
              ...
              {[1, 2, 3, 4].map(t => {
                ...
                return (
                  <div key={t} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ color: TIER_COLORS[t], fontSize: 11, letterSpacing: 1 }}>T{t} {tx.tier_labels[t]}</span>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 8 }}>
              {filteredSources.length === 0
                ...
                : filteredSources.map(s => <SourceCard key={s.id} source={s} compact={compact} tierLabel={tx.tier_labels[s.tier]} />)
              }
            </div>
          </>
        )}
        ...
        {activeTab === "ai" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, padding: "9px 16px", background: "#0a120a", border: "1px solid #1a2a1a", borderRadius: 2, flexWrap: "wrap" }}>
              <span style={{ color: liveMode ? "#44ff88" : "#445544", fontSize: 12, letterSpacing: 2, animation: liveMode ? "pulse 1.5s infinite" : "none" }}>
              ...
              <button onClick={() => setLiveMode(v => !v)} style={{
                ...
                color: liveMode ? "#44ff88" : "#556655", padding: "5px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, borderRadius: 2,
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
              {tx.sample_qs.map(q => (
                <button key={q} onClick={() => { setAiQuery(q); setTimeout(() => aiInputRef.current?.focus(), 50); }} style={{
                  ...
                  padding: "7px 13px", cursor: "pointer", fontFamily: "monospace", fontSize: 13, borderRadius: 2,
                }}
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <textarea ref={aiInputRef} value={aiQuery} onChange={e => setAiQuery(e.target.value)}
                ...
                style={{ flex: 1, background: "#0d140d", border: "1px solid #2a3a2a", color: "#c8d4b8", padding: "14px 18px", fontFamily: "monospace", fontSize: 15, lineHeight: 1.7, borderRadius: 2, resize: "vertical" }}
              />
              <button onClick={handleAiQuery} disabled={aiLoading || !aiQuery.trim()} style={{
                ...
                fontFamily: "monospace", fontSize: 13, letterSpacing: 2, borderRadius: 2, minWidth: 110,
            ...
            {aiLoading && fetchSteps.length > 0 && (
              <div style={{ background: "#0a120a", border: "1px solid #1a2a1a", borderRadius: 2, padding: "12px 16px", marginBottom: 12 }}>
                {fetchSteps.map((step, i) => (
                  <div key={i} style={{ color: i === fetchSteps.length - 1 ? "#44ff88" : "#2a4a2a", fontFamily: "monospace", fontSize: 12, lineHeight: 2, letterSpacing: 1 }}>
            ...
            {aiResponse && (
              <div style={{ background: "#0d140d", border: "1px solid #1a3a1a", borderLeft: "3px solid #44ff88", padding: "18px 22px", borderRadius: 2 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  <span style={{ color: "#445544", fontSize: 12, letterSpacing: 2 }}>{tx.analyst_resp}</span>
            ...
            {!aiResponse && !aiLoading && (
              <div style={{ color: "#2a3a2a", textAlign: "center", padding: "60px 0", fontSize: 13 }}>
        ...
        {activeTab === "sanitize" && (
          <div>
            <div style={{ marginBottom: 14, padding: "10px 16px", background: "#120a00", border: "1px solid #2a1a00", borderLeft: "3px solid #ffaa00", borderRadius: 2 }}>
              <div style={{ color: "#ffaa00", fontSize: 12, letterSpacing: 2, marginBottom: 4 }}>{tx.sanitizer_badge}</div>
              <div style={{ color: "#998855", fontSize: 14, lineHeight: 1.7 }}>{tx.sanitizer_desc}</div>
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ color: "#556655", fontSize: 12, letterSpacing: 1, marginBottom: 6 }}>{tx.sample_qs_label}</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {SAMPLE_QUOTES.map(sq => (
                  <button key={sq.label} onClick={() => { setQuoteInput(sq.text); setQuoteActor(sq.actor); }} style={{
                    ...
                    padding: "6px 12px", cursor: "pointer", fontFamily: "monospace", fontSize: 12, borderRadius: 2,
                  }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 3fr", gap: 8, marginBottom: 8 }}>
              <input value={quoteActor} onChange={e => setQuoteActor(e.target.value)} placeholder={tx.actor_ph}
                style={{ background: "#0d0a00", border: "1px solid #2a1a00", color: "#c8b488", padding: "11px 16px", fontFamily: "monospace", fontSize: 13, borderRadius: 2 }} />
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              <textarea value={quoteInput} onChange={e => setQuoteInput(e.target.value)}
                ...
                style={{ flex: 1, background: "#0d0a00", border: "1px solid #2a1a00", color: "#c8b488", padding: "14px 18px", fontFamily: "monospace", fontSize: 14, lineHeight: 1.7, borderRadius: 2, resize: "vertical" }}
              />
              <button onClick={handleSanitize} disabled={quoteLoading || !quoteInput.trim()} style={{
                ...
                fontFamily: "monospace", fontSize: 13, letterSpacing: 2, borderRadius: 2, minWidth: 120,
            ...
            {quoteLoading && (
              <div style={{ background: "#120a00", border: "1px solid #2a1a00", borderRadius: 2, padding: "18px 22px", textAlign: "center" }}>
                <div style={{ color: "#ffaa00", fontSize: 12, letterSpacing: 2, animation: "pulse 1s infinite" }}>{tx.cross_ref}</div>
              </div>
            )}
            ...
            {quoteResult && !quoteLoading && (
              <div>
                ...
                {quoteResult.raw && <div style={{ background: "#120a00", border: "1px solid #2a1a00", borderLeft: "3px solid #ffaa00", padding: 16, borderRadius: 2, color: "#c8b488", fontSize: 14, lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{quoteResult.raw}</div>}
                {quoteResult.sanitized_version && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div style={{ background: "#0d0a00", border: "1px solid #2a1a0055", borderLeft: "3px solid #ff4444", padding: "14px 18px", borderRadius: 2 }}>
                        <div style={{ color: "#ff4444", fontSize: 11, letterSpacing: 2, marginBottom: 8 }}>{tx.orig_core}</div>
                        <div style={{ color: "#d4b898", fontSize: 14, lineHeight: 1.8 }}>{quoteResult.original_core_claim}</div>
                      </div>
                      <div style={{ background: "#0a0d0a", border: "1px solid #1a3a1a", borderLeft: "3px solid #44ff88", padding: "14px 18px", borderRadius: 2 }}>
                        <div style={{ color: "#44ff88", fontSize: 11, letterSpacing: 2, marginBottom: 8 }}>{tx.sanitized}</div>
                    ...
                    {quoteResult.propaganda_elements?.length > 0 && (
                      <div style={{ background: "#0d0a00", border: "1px solid #2a1a00", borderRadius: 2, padding: "14px 18px" }}>
                        <div style={{ color: "#ffaa00", fontSize: 11, letterSpacing: 2, marginBottom: 10 }}>{tx.prop_el(quoteResult.propaganda_elements.length)}</div>
                    ...
                    {quoteResult.contradictions?.length > 0 && (
                      <div style={{ background: "#0d0a0a", border: "1px solid #3a1a1a", borderRadius: 2, padding: "14px 18px" }}>
                        <div style={{ color: "#ff4444", fontSize: 11, letterSpacing: 2, marginBottom: 10 }}>{tx.contras(quoteResult.contradictions.length)}</div>
                    ...
                    {quoteResult.use_with_caution?.length > 0 && (
                      <div style={{ background: "#0a0a0d", border: "1px solid #1a1a3a", borderRadius: 2, padding: "12px 16px" }}>
                        <div style={{ color: "#44aaff", fontSize: 11, letterSpacing: 2, marginBottom: 8 }}>{tx.caution}</div>
            ...
            {!quoteResult && !quoteLoading && (
              <div style={{ color: "#2a2000", textAlign: "center", padding: "50px 0", fontSize: 13 }}>
        ...
        {activeTab === "verify" && (
          <div style={{ maxWidth: 700 }}>
            <div style={{ marginBottom: 20, color: "#556655", fontSize: 13, letterSpacing: 1 }}>{tx.verify_sub}</div>
            <div style={{ marginBottom: 24 }}>
              <div style={{ color: "#ffaa00", fontSize: 13, letterSpacing: 2, marginBottom: 10 }}>{tx.checklist_title}</div>
              {VERIFY_CHECKLIST[lang].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 12, padding: "9px 0", borderBottom: "1px solid #0d1a0d" }}>
                  <span style={{ color: "#ffaa00", fontWeight: 700, minWidth: 24 }}>{String(i + 1).padStart(2, "0")}</span>
                  <span style={{ color: "#b8c8aa", fontSize: 14, lineHeight: 1.7 }}>{item}</span>
                </div>
              ))}
            </div>
            <div style={{ marginBottom: 24 }}>
              <div style={{ color: "#ff4444", fontSize: 13, letterSpacing: 2, marginBottom: 10 }}>{tx.reliability_title}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[
                  ...
                  <div key={label} style={{ background: "#0d140d", border: "1px solid #1a2a1a", padding: "11px 14px", borderRadius: 2 }}>
                    <div style={{ color: "#aab8aa", fontSize: 12, marginBottom: 6 }}>{label}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ color, fontSize: 13, fontWeight: 700, minWidth: 38 }}>{rel}%</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div style={{ color: "#44aaff", fontSize: 13, letterSpacing: 2, marginBottom: 10 }}>{tx.geo_title}</div>
              {[
                ...
                >
                  <span style={{ color: "#55bbff", minWidth: 170, fontSize: 14 }}>{t.name}</span>
                  <span style={{ color: "#7a8870", fontSize: 13 }}>{t.desc}</span>
                </a>
              ))}
            </div>
          </div>
